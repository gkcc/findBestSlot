...
def test_current_analysis_report_answers_acceptance_questions():
    ...
    assert "| 策略 | 目标套装 | 位置 | 主属性 | 固定副属性 | horizon | immediate_EV | option_EV | horizon_EV | 期望提升 | 质量提升 | 有效提升 | 母盘/次 | 校音器/次 | 共鸣核/次 | 质量/母盘 | 有效/母盘 | 排序向量/母盘 | 相对随机 |" in report
    assert "| 随机位置 | 云岿如我 | 1-6 随机 |" in report
    assert "| 固定位置 | 折枝剑歌 | 6号位 |" in report
    assert "优于随机，才建议固定" in report
...
def test_first_version_acceptance_report_answers_all_core_questions():
    ...
    assert "| 1 | localhost 形态能不能做成 App | 已提供桌面壳、Windows exe 打包和窗口关闭自动收尾 |" in report
    assert "独立本地 profile" in report
