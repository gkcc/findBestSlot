...
当前重点支持《绝区零》星徽・比利驱动盘分析，并已接入《崩坏：星穹铁道》的遗器/位面饰品规则、套装文本、固定位置合成和自塑尘脂机会成本口径。其它游戏、代理人/角色和目标模板仍可通过 YAML 扩展游戏规则、计算目标、概率模型和示例盘面。
...
内部迁移版 Windows 产物使用独立的 backend 与 Action EV worker sidecar：
...
1. 在顶部选择游戏、代理人、目标模板和概率模型。
   - “代理人”入口就在顶部表单里，显示为“切换代理人”按钮；代理人只负责展示身份和数据归属，实际计算使用右侧关联的目标模板。
   - 目标模板不是装备模板；它只定义计算目标：每个位置期望的主属性、期望达成的 4+2/2+2+2 套装结构、副属性有效排序和并列关系。
...
当前装备必须先确认才允许计算。库存变化、当前装备变化、游戏/代理人/目标模板/概率变化都会清空旧结果，避免拿过期盘面继续算。
...
结果页先展示推荐卡，主口径是推荐动作、目标套装/位置、主属性、固定副属性、horizon、有效期望、有效/母盘、比较口径和“精确”计算口径。随机位置行由同目标套装各固定位置 action value 按位置概率加权平均得到，不存在唯一典型搭配。Action EV 明细默认显示前 20 条，可点击“显示全部”展开完整精确结果用于审计；质量向量仍保留在算法和报告审计层，不作为桌面推荐卡主展示。

## 星铁口径

- 星铁关闭随机位置合成；Action EV 只生成固定位置、固定位置 + 固定主属性、固定位置 + 固定主属性 + 固定副属性。
- 外圈遗器只允许头部、手部、躯干、脚部；位面饰品只允许位面球、连结绳。默认目标模板是 `识海迷坠的学者 4 + 繁星竞技场 2`，用于通用暴击审计，不代表所有角色毕业推荐。
- 当前配置包含 60 套星铁文本素材：32 套隧洞遗器、28 套位面饰品；不内置联网图片，缺图时自动回退文字。
- 初始 3 词条、+3 前的星铁遗器可以在编辑器里记录“预告第 4 副属性”；保存到 YAML 时字段为 `revealed_next_substat`。该能力由游戏配置 `enhancement.revealed_next_substat_supported` 控制，当前只在 HSR 打开；候选胚子评估、库存升级机会和当前最优搭配的强化期望都会按这个已知副属性条件化，不再把第 4 条当随机未知。
- 星铁和绝区零代理人 catalog 使用真实代理人/角色条目和本地图，选择器默认按实装版本从新到旧展示，并支持按属性、职介和名称搜索筛选；代理人只关联目标模板，不等同于目标模板。
