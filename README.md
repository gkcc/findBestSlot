...
库存维护不需要手动编辑 YAML。普通流程是：当前装备页维护身上的 6 件；调律策略页打开“背包库存（点这里添加未装备盘）”；点击“添加库存件”；选择位置、套装、主属性、等级和副属性；最后点击“保存库存到本机”。之后攻略结论和 Action EV 会自动把这些未装备成品 / 胚子纳入理论期望。导入 / 导出库存 YAML 只作为备份、迁移或给别人复现实验使用。
...
“随机 vs 固定位置收益效率”会按当前 6 件盘估算收益：随机和固定都会把新盘加入库存后重求当前套装约束下的最优组合，并同时展示质量提升 / 母盘和原始有效词条提升 / 母盘。
Action EV 展望步数选 2 时仍是完整概率分布的精确理论计算，不是抽样模拟。为避免页面控件一变化就自动长时间重算，界面会先显示 horizon=1 快速结论；需要 horizon=2 时点击“开始精确计算 horizon=2”，页面会显示 action 进度、DP 状态数和缓存命中数。
“特殊资源全局边际 EV”是审计明细，默认不自动计算；只有需要复核校音器 / 共鸣核边际差值时，勾选“计算特殊资源全局边际 EV 详情”。
